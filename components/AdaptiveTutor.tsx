...
const AdaptiveTutor: React.FC = () => {
    ...
    const handleSend = async () => {
        ...
        let fileDataForApi: { mimeType: string; data: string } | undefined = undefined;

        if (file) {
            if (file.type === 'application/pdf') {
                try {
                    const pdfText = await extractPdfText(file);
                    promptForApi = `Based on the content of the attached PDF (${file.name}), please answer the following question: ${input}\n\nPDF CONTENT:\n${pdfText}`;
                    userMessage.file = { name: file.name, type: file.type, base64Data: '' }; // No preview for PDF
                } catch (e) {
                ...
                try {
                    ...
                    fileDataForApi = { mimeType: file.type, data: base64Data };
                    userMessage.file = { name: file.name, type: file.type, base64Data };
                } catch (error) {
        ...
        try {
            ...
            if (match && match[1]) {
                try {
                    ...
                    aiResponseText = aiResponseText.replace(timelineRegex, '').trim();
                } catch (e) {
                    console.error("Failed to parse timeline entry from AI response:", e);
                }
            }
            
            const aiMessage: Message = { id: uuidv4(), text: aiResponseText, sender: 'model' };
            setTutorHistory([...updatedHistory, aiMessage]);
        } catch (error) {
            console.error(error);
            const errorMessage: Message = { id: uuidv4(), text: "Sorry, I'm having trouble connecting. Please try again.", sender: 'model' };
            setTutorHistory([...updatedHistory, errorMessage]);
        } finally {
            setIsLoading(false);
        }
    };
